package com.indaco.bart.ui.screens.details
...
class BartMapsFragment : Fragment(R.layout.fragment_bart_maps) {
    ...
    private val viewModel: BartDetailsViewModel by viewModels({this},{viewModelFactory})
    private val args by navArgs<BartMapsFragmentArgs>()
    private val binding by viewBinding(FragmentBartMapsBinding::bind)
...
//    }
    ...
    private fun toggleViewFlipperAnimation(child: Int) {
        with(bottomBinding.viewFlipper) {
            if (child == 0) {
                setInAnimation(context, appR.anim.slide_in_from_left)
                setOutAnimation(context, appR.anim.slide_out_to_right)
            } else {
                setInAnimation(context, appR.anim.slide_in_from_right)
                setOutAnimation(context, appR.anim.slide_out_to_left)
            }
        }
    }
}